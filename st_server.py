...
class StockTickerGame():
    ...
    def add_player(self, client):
        if client.name in self.players:
            raise RuntimeError(f"{client.name} already in this game")
        with self.game_lock:
            client.game = self
            p = Player( client, 
                        self.INIT_CASH,
                        (0 for s in stock_names))
            self.players[player.name] = p
            client.set_player_info(p)

    def remove_player(self, playername):
        with self.game_lock:
            client = self.players[playername].client
        ...
        with self.game_lock:
            ...
            if self.option_ignore_nopay_divrolls:
                while self.roll_div_unpaid(roll) and attempts < max_attempts:
                    ...
                    attempts += 1
                    roll = self.die_roll()
            player_msgs.extend((p, bmsg('roll', roll)) for p in self.players.values())
            ...
            if action in ['UP', 'DOWN']:
                ...
                player_msgs.extend((p, pc_msg) for p in self.players.values())
                if split_bust_msgs:
...
def process_message(message, clientname):
    ...
    game = player.game # TODO: this will cause havoc if/when not in a game
    if message['TYPE'] == 'msg':
        ...
        if bln_success:
            ...
            game = games[gamename]
            player = game.get_player(client.get_name())
            client.conn.send(bmsg('initgame', game.init_game_info(player)))
        else:
            # fail
...
def disconnect_client(player):
    ...
    with lock: #Unsure how necessary or correct in logic this is...
        ...
        if player.game:
            ...
            send_others_game(bmsg('disconnect', player.name))
            send_others_game(bmsg('playerlist', tuple(game.players.keys())))
            print (f'Removed {player.name} from game "{game.name}"')
        ...
        for key, mask in events:
            if key.data == "listen-new":
                ...
                if validconnection:
                    ...
                    conn.send(bmsg('conn-accept', ((g.name,g.id) for g in games.values())))
